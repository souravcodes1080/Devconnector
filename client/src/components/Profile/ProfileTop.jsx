...
function ProfileTop({
  ...
  // Step 1: Save name & avatar fallback from profile.user
  ...
  useEffect(() => {
    if (profile?.user?.name && profile?.user?.avatar) {
      setSavedUser({
        name: profile.user.name,
        avatar: profile.user.avatar,
      });
  ...
  // Step 2: If profile or user data vanishes, fallback to saved
  if (!profile || (!profile.user && !savedUser.name)) {
    const fallbackAvatar = savedUser.avatar || user?.avatar;
    const fallbackName = savedUser.name || user?.name;
  ...
  // Step 3: Destructure user with fallback
  const {
    status,
    company,
    location,
  ...
  const name = profileUser?.name || savedUser.name;
  const avatar = profileUser?.avatar || savedUser.avatar;
  ...
  return (
    <>
      ...
      <div className="px-5 py-5 my-4 bg-white border shadow-sm md:py-10 md:px-10 profile-about bg-light border-gray-300/50 rounded-xl">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl">{name?.split(" ")[0]}'s Bio</h2>
          {auth.isAuthenticated &&
            auth.loading === false &&
            auth.user._id === id && (
              <FiEdit3
          ...
          experience.map((exp, i) => (
            <div key={exp._id}>
              <div className="flex items-center justify-between">
                ...
                <FaTrash
                  className="cursor-pointer text-[12px] mr-1 hover:text-blue-700"
...
export default connect(mapStateToProps, { deleteExperience, deleteEducation })(
  ProfileTop
);
